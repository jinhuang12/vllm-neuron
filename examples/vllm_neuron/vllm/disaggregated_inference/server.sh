...
if [ "$PROXY" = "1" ]; then
    ...
    PROXY_PORT=8000
    # 1P1D setup
    # The vanilla toy_proxy_server only supports single host setup. had to update it so it supports multi-host
    PROXY_CMD="python3 /shared/yzdou/workspace/vLLM Neuron/main/toy_proxy_server.py --port $PROXY_PORT"
