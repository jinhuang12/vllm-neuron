...
"""Accuracy debugger example — GPT-OSS-20B BF16.
...
GPT-OSS is a Mixture-of-Experts model, so the tensor-compare thresholds are
tuned for expert-routing divergence (see comments below).

Usage:
    # Full pipeline (default) — launches its own server:
    python run_accuracy_debugger_gpt_oss.py --model /path/to/gpt-oss-20b

    # Task analysis only:
    python run_accuracy_debugger_gpt_oss.py --model /path/to/model --mode task_only

    # Prompt analysis only (reuses deviated prompts from a prior task run):
    python run_accuracy_debugger_gpt_oss.py --model /path/to/model \
        --mode prompt_only --output-dir ./accuracy_report
...
# GPT-OSS tensor compare config: last 3 layers + embed/norm/lm_head
NUM_LAYERS = 24
...
def _build_serve_cmd(model: str, port: int) -> str:
    hf_overrides = json.dumps({"quantization_config": {}})
    return (
        f"vllm serve {model}"
...
def _run_prompts(model: str, prompts: list, output_dir: str):
    ...
    return run_prompt_stage(
        ...
        prompts=prompts,
        plugin_steps=[
            LogitValPlugin(tol_map=GPT_OSS_TOL_MAP),
            KvCachePlugin(),
            TensorComparePlugin(
                modules=TENSOR_COMPARE_MODULES,
                module_order=_module_order(),
                tp_size=TP_SIZE,
                # Plugin-level threshold (prefill+decode) is relaxed because MoE
                # routing divergence causes high prefill L2 at specific tokens.
                # The assertion below enforces a stricter 3.0x on decode only.
                max_l2_ratio=6.0,
            ),
        ],
...
def main(argv: Optional[Sequence[str]] = None) -> PipelineResult:
    ...
    pass/fail (that verdict — which thresholds matter, the MoE-aware
    tensor-compare gating — lives in ``test_accuracy_debugger.py``). It still
    raises on *operational* failures that stop the pipeline from proceeding (for
    example, a server it cannot stop before the offline phase)."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--model", required=True, help="Path to the GPT-OSS-20B checkpoint."
    )
    parser.add_argument(
        "--mode",
