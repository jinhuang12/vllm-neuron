...
def tp_sum_int(value: int, timeout: timedelta | None = None) -> int:
    """Sum one integer across the TP group, synchronizing the ranks as it does.

    Every rank has to reach the all-reduce, so this is a TP barrier that also
    carries a value: the caller learns the group's total, not just that every
    rank arrived.
    """
    import torch
    import torch.distributed as dist
