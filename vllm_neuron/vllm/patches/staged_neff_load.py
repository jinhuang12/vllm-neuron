...
"""Stage the load of a compiled graph so one wave of ranks loads at a time.

Building the executable for a graph loads the whole graph into host memory,
while the execution that follows holds far less. A tensor-parallel group that
builds on every rank at one instant therefore exhausts the host long before it
runs anything. This module brackets the compiler the plugin registers with
torch.compile and lets the ranks through it in waves. The bracket includes the
compiler's own cache lookup, which is cheap on a hit; a miss inside it
serializes a compilation as well as a load.

The signals between waves point forward only: a wave waits for the wave before
it and is never waited on again. By the time a wave has signalled, its own ranks
are inside the forward's collectives, where a group-wide wait would deadlock.
Each rank writes its flag after the compiler returns, so a rank that blocks on
the way out holds nobody up, and a wave that never signals costs the next wave
one bounded wait rather than the whole run.

Each load gets its own flag set, named by the run, the phase, the bucket and the
load's ordinal on this rank. A set shared across a warmup window, which builds
several graphs, would let every graph after the first pass the wait at once and
load on the whole group together, which is what the waves prevent. The ordinal
needs no collective: every rank runs the same warmup code and builds the same
graphs in the same order. The first wave of a later load follows the last wave of
the load before it, so the ranks inside a compiler at any instant stay down to
one wave across a whole window.

Staging is armed only for a load a caller has named; anything else passes
straight through.
"""
...
def pin_this_run() -> str:
    """Name this run once, so its ranks never read a flag an earlier run left behind."""
    if not envs.VLLM_NEURON_NEFF_LOAD_SIGNAL_DIR:
        return ""
    token = os.environ.get(RUN_TOKEN_ENV)
    if not token:
        # Set here so every worker this process starts inherits it, as the plugin
        # already does for the prometheus directory. A rank that reads no token
        # falls back to one shared name, which still keeps the ranks of one run
        # together; the name it used is printed in the neff_load_root row.
...
def name_this_load(phase: str, bucket: str) -> None:
    """Name the warmup call the graph loads that follow belong to."""
...
def install_staged_compiler(registry, name: str, compile_fn):
    """Hold this backend name with the bracketed compiler, wrapping the current one."""
...
def stage_load(compile_fn, *args, **kwargs):
    ...
    try:
        ...
        # The row and the flag are written even when the compilation raises,
        # because a wave that never signals leaves the wave behind it waiting
...
def _registered_compiler(registry, name: str):
    """Return the compiler this backend name holds, or None when it is free."""
    try:
...
def _forget_registration(registry, name: str) -> None:
    """Drop this name from the registry's tables, which refuse to re-register it."""
...
def _rank_and_world() -> tuple[int | None, int | None]:
    """Read this rank and the world size off the process group, as the compiler does."""
...
def _run_root() -> str | None:
    """Return the directory this run signals in, or None when none is configured."""
...
def _wave_in_front(name_root: str, wave: int, waves: int, load: int):
    """Return the load directory and wave this wave follows, or None when it is first.

    A later wave of one load follows the wave before it. The first wave of a
    later load follows the last wave of the load before it, which keeps the ranks
    inside a compiler at any instant down to one wave across a window that builds
    several graphs.
    """
    if wave:
...
def _wait_for_the_wave_in_front(
    ...
    """Wait for every rank of the wave in front to signal, or give up and proceed."""
...
def _rss_anon_kib() -> int:
    """Read this process's anonymous resident size in KiB, or zero without /proc."""
    try:
