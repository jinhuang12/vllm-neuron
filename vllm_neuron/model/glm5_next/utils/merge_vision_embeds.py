...
Follows ``vllm_neuron/model/qwen3_vl/utils/merge_vision_embeds.py``, with two
differences that come from what GLM-5.3-Flash's tower emits:

* **No deepstack.** Qwen3-VL concatenates one merged tensor per
  ``deepstack_visual_indexes`` entry onto the main embedding, so its cache rows
  are ``out_hidden_size * (1 + num_deepstack)`` wide and its merge splits them
  apart again. GLM-5.3-Flash's tower returns a single ``pooler_output`` of width
  ``out_hidden_size``, so a cache row here is exactly as wide as the decoder's
  hidden state.
* **A width mismatch raises instead of slicing.** Because the two widths are the
  same number by construction, a row that is not ``hidden_size`` wide means the
  tower and the decoder disagree; truncating it would hide that.

The sequence-parallel coordinate remap and the dummy-row scatter are branch-free
so the prefill graph traces under ``torch.compile``.
"""
