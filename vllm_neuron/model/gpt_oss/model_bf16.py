...
class GptOssAttention(nn.Module):
    ...
    def forward(
        ...
        - Decode cross-DP EP at B*S>=128 (``rsag_dispatch``): exit with
          ReduceScatter[TP] instead of AllReduce[TP], leaving tokens
          sequence-parallel for the MoE DP->EP AllGather[World] dispatch.
        """
        ...
        if kv_segment_size:
            ...
            if not self.fp8_packed:
                ...
                q_hbm, _, _ = NF.qkv_proj(
                    ...
                    slot_mapping=slot_mapping_clamped.unsqueeze(0),
                )
                q = (
                    q_hbm.squeeze(0)
            ...
            # FP8 KV dequant for the segmented (chunked) prefill path: the cache
            # stores k*k_scale / v*v_scale (see _write_paged_kv_cache), so the
            # prior-token K/V the kernel reads back are quantized and must be
            # dequantized. Mirror this layer's own forward_decode, without relying
            # on the kernel's k_scale/v_scale args (their dequant convention differs
            # between the decode and segmented kernels; the segmented CPU fallback
            # ignores them and expects a caller-folded scale): fold the K-dequant
            # into the softmax scale (scale = scaling / k_scale_float, as decode's
            # softmax_scale) and the V-dequant out of the attention output — the
            # separate-o_proj equivalent of decode's W_out = o_proj_weight /
            # v_scale_float. Both are exact scalar folds and no-ops at
    ...
    def forward_decode(
        ...
        TP all-reduce is done after the kernel — except in the cross-DP EP
        ``rsag_dispatch`` case, where the O-proj partials are ReduceScatter[TP]'d
        (not AllReduce'd) so the output stays sequence-parallel for the MoE.
        ...
        # rsag_dispatch: ReduceScatter[TP] (sum + shard to [T/tp, H]) so the MoE
        # dispatches the shrunk tensor via AllGather[World]. Else AllReduce[TP].
        if rsag_dispatch:
...
class GptOssExperts(nn.Module):
    ...
    def forward_decode(
        ...
        >>> PARALLELISM: Cross-DP EP dispatch/combine. rsag_dispatch (B*S>=128)
        >>>   swaps AllGather[DP]/AllReduce[World] for AllGather[World]/
        >>>   ReduceScatter[World]; the layer's AllGather[TP] restores the layout.
        """
        ...
        # rsag_dispatch: ReduceScatter[World] (stays sequence-parallel; the
        #   layer's AllGather[TP] restores it). Else AllReduce[World] + DP slice.
        if self.cross_dp_ep:
            if rsag_dispatch:
...
class GptOssDecoderLayer(nn.Module):
    ...
    def _use_rsag_dispatch(self, hidden_states: torch.Tensor) -> bool:
        ...
        so the residual slice is a plain narrow (GPT-OSS runs both == 1).
        ``hidden_states`` is the TP-replicated [B_local*S, H] decode tensor, so
        global B*S = shape[0] * dp_size (== the combine's ``output.shape[0]``).
        """
        experts = self.mlp.experts
        if not experts.cross_dp_ep or self.attn_dp != self.mlp_dp:
            return False
    ...
    def forward(
        ...
        # so the paired collectives agree.
...
class GptOssForCausalLM(nn.Module, SupportsEagle3):
    ...
    def bind_kv_cache(self, kv_caches: dict[str, list[torch.Tensor, torch.Tensor]]):
        ...
        for i, layer in enumerate(self.model.layers):
            ...
            # (one rank higher than V, which is never packed). The hasattr guard
            # keeps this safe for any attention class that predates the flag.
