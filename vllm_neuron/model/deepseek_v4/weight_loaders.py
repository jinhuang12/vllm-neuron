...
def _fp8_pow2_expert_weight_loader(
    ...
    RESTORED by LD-80 (plan §21.2, iteration 30): the LD-67 bf16 fold is
    retired — the loader STOPS at the LD-23 bit-exact fp8-per-channel
    intermediate, which is the operator's own recorded expert container
    ("fp4 experts upcast to fp8", production-config 2026-08-18). This is the
    pre-LD-67 loader verbatim (542cf85~1); the fold's justification was void
    on the record (F-218: the ep18 flood had three serial b2e-stream
    consumers the fold could not touch; the LD-73 sentinel mask on the index
    stream itself is what contains that class, and it STANDS).
...
def _fp8_pow2_expert_scale_loader(
    ...
    RESTORED by LD-80 (plan §21.2) together with
    :func:`_fp8_pow2_expert_weight_loader` — see its header for the
    residency-only rationale.
...
# RETIRED (LD-80, plan §21.2): ``_bf16_folded_expert_weight_loader`` and its
# ``_LEGACY_E4M3_DECODE_TABLE`` folded the per-channel power-of-two scale into
# the value and emitted bf16 (LD-67, Amendment 7). The fold cost +4.31 GiB of
# resident weight I/O per core and its flood-class justification was void on
# the record (F-218); LD-80 restores the operator's recorded fp8 container.
# The LD-67 code is recoverable at 542cf85 if the operator ever re-approves
# the fold as a deviation.
...
def attach_moe_loaders(
    ...
    * **Routed experts**: expert-parallel, ``n_routed_experts // ep_degree``
      local experts (4 of 256 at EP=64). LD-23 (residency restored by LD-80):
      the checkpoint's MXFP4 weights and group-32 E8M0 scales are REQUANTIZED
      here to legacy-E4M3 bytes plus one fp32 power-of-two scale per output
      channel, and the bytes are written transposed to ``[in, out]``. Both
      parameters read both checkpoint tensors; see the LD-23 section header
      for why the fold is bit-exact and why it raises instead of rounding.
    * **Shared expert**: sharded over a ``shared_tp_size``-way subgroup and
    ...
    #
    # BOTH loaders per leaf take BOTH checkpoint tensors: the requantization
    # folds the E8M0 group scales into one power-of-two per output channel,
    # so weight bytes and scale are computable only from the pair. The key
    # list is therefore ``weights then scales``: two groups of ``n_experts``,
    # which is exactly the layout ``expert_parallel_grouped_loader`` documents
    # and trims group-wise (``utils/weight_loader.py:697-714``).
    ...
    *,
    ...
    def add_block(param_layer: str, key_prefix: str, *, layer_idx: int) -> None:
        ...
        for leaf in ("w1", "w3", "w2"):
            # LD-23 (residency restored by LD-80): BOTH routed-expert
            # parameters per leaf read BOTH checkpoint tensors, because the
            # requantization folds the E8M0 group scales into one
            # power-of-two per output channel — weight bytes and scale are
            # computable only from the pair. The order is ``all weights then
            # all scales`` -- the two-group layout ``expert_parallel_grouped_
