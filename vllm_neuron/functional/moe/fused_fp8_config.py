...
def select_tiles(hidden, intermediate, rows, block_m=None, block_n=None, block_k=None):
    """Return (M,N,K) scheduling groups; each scaled product stays128x128.

    Defaults bound the live gate weight group to8MiB and use a conservative
    24MiB workspace estimate. Explicit settings are checked for ISA/scale
