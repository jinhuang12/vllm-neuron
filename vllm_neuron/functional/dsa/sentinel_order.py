...
compiler to choose the scatter source's layout, and its InsertOffloadedTransposes pass chose a
transposed one -- the 128-wide axis moved last, one DMA transpose per DSA layer on the prefill path.
The kernel emits no scatter, so that pass has nothing to lay out.
...
tiles fit one SBUF partition -- so neither the padded width nor any value the kernel computes
exceeds a few thousand, far below the 2**24 an fp32 holds exactly. A wider row takes the torch path.
"""
...
def _sentinel_order_nki(pool_ids_hbm):
    ...
    # The search reads eight values at a time, so the width is padded to a multiple of eight with
    # trailing sentinel columns: they sort after every real column and after every original
    # sentinel, so the first k ordered columns are unchanged and the pad is never stored.
    k_pad = -(-k // SEARCH_WIDTH) * SEARCH_WIDTH
...
def _record_nki_dispatch(rows: int, k: int) -> None:
    ...
    Dynamo runs a folded call once at trace time, so the log record is written on the host and the
    ``logging.Logger`` call never enters the graph the runner compiles with ``fullgraph=True``. A
    folded helper takes ints only, so the kernel is read as a module global rather than passed in.
    """
...
def dsa_sentinel_order(pool_ids: Tensor) -> Tensor:
    """``[rows, select_k]`` pool ids, the negatives moved to the trailing columns.
    ...
    the torch path. Raises ``SentinelOrderError`` for a tensor that is not 2-D.
    """
...
def _dsa_sentinel_order_torch(pool_ids: Tensor) -> Tensor:
    """CPU reference and fallback: the cumsum-and-scatter spelling the kernel replaces."""
