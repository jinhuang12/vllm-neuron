...
def predicated_folded_load(
    ...
    """
    kernel_assert(
        len(data_hbm.shape) == 2, "Expected input tensor to have shape [B, N]"
    )
    ...
    if data_sb == None:
        return_out = True
        data_sb = nl.ndarray(
            (batch_size_sharded * fold_factor, n_folded),
            dtype=data_hbm.dtype,
            buffer=nl.sbuf,
        )
    ...
    if sb_cols > n_folded:
        nisa.memset(
            dst=data_sb[:, nl.ds(n_folded, sb_cols - n_folded)], value=fill_value
        )
    ...
    if n == fold_factor * n_folded:
        ...
        batch_bound = min(
            batch_size_sharded * fold_factor, batch_size * fold_factor - base_offset
        )
        ix, iy = nl.ds(0, batch_bound), nl.ds(0, n_folded)
        ix_src = nl.ds(base_offset, batch_bound)
        nisa.dma_copy(
            src=src_hbm_reshape[ix_src, iy],
            dst=data_sb[ix, iy],
    ...
    nisa.memset(
        dst=data_sb[:, nl.ds(remainder, n_folded - remainder)], value=fill_value
    )
    ...
    for batch_line_idx in nl.affine_range(batch_size_sharded_bounded):
        ...
        base_idx = row_idx * n
        ix_0_dst, iy_0_dst = (
            nl.ds(batch_line_idx * fold_factor, fold_factor),
            nl.ds(0, remainder),
        )
        ix_1_dst, iy_1_dst = (
        ...
        nisa.dma_copy(
            dst=data_sb[ix_0_dst, iy_0_dst],
            src=src_hbm_flat.ap(pattern=src_ap_0, offset=base_idx),
        )
        nisa.dma_copy(
            dst=data_sb[ix_1_dst, iy_1_dst],
            src=src_hbm_flat.ap(pattern=src_ap_1, offset=base_idx + remainder),
        )
    if return_out:
        return data_sb
    return None
...
def unfolded_store(
    ...
    batch_size_sharded_bounded = min(
        batch_size_sharded, batch_size - dst_batch_line_offset
    )
    for batch_line_idx in nl.affine_range(batch_size_sharded_bounded):
        ...
        nisa.dma_copy(
            src=sbuf[ix_0_src, iy_0_src],
            dst=data_hbm_flat.ap(pattern=dst_ap_0, offset=base_idx),
        )
        nisa.dma_copy(
            src=sbuf[ix_1_src, iy_1_src],
            dst=data_hbm_flat.ap(pattern=dst_ap_1, offset=base_idx + remainder),
        )
