...
def reduce(op: str = "mul", input_list: Optional[List] = None, initial_value=None):
    ...
    kernel_assert(
        op in supported_ops, f"only ops in {supported_ops} are supported, got {op}"
    )
...
class TopkConfig(nl.NKIObject):
    ...
    def cost_estimate(self) -> int:
        ...
        """
        return (
            div_ceil(self.k, HW_PARAMS.dve_max_alus)
            * 2
            * (self.vocab_size + HW_PARAMS.fixed_dve_inst_overhead)
        )
...
def create_topk_config(
    ...
    if BxS == 1:
        logger.info(
            f"Setting num_programs to 1 since {BxS}, user specified num_programs {n_prgs}"
        )
        prg_id = 0
        n_prgs = 1
    ...
    kernel_assert(
        config.vocab_size_valid(), f"topk expects vocab_size ({vocab_size}) >= k ({k})"
    )
    return config
...
class RotationalTopkConfig(nl.NKIObject):
    ...
    def assert_valid(self) -> None:
        kernel_assert(
            self.inp_shape_valid(),
            f"topk expects input to be at least 2D, got ({self.inp_shape})",
        )
        kernel_assert(
            self.vocab_size_valid(),
            "topk expects vocab_size ({self.vocab_size}) >= k ({self.orig_k}),",
        )
...
def _calculate_rotational_constants_baseline(
    ...
    local_top_k_per_stage = get_ceil_aligned_size(
        div_ceil(orig_k, n_stages), HW_PARAMS.topk_per_stage
    )
...
def _estimate_rotational_cost(
    orig_k: int, vocab_size: int, ns: int, sorted: bool = True
) -> float:
    ...
    for stage_idx in range(ns):
        topk_cost += (
            div_ceil(lk, 8)
            * 2
            * (sf + lk * stage_idx + HW_PARAMS.fixed_dve_inst_overhead)
        )
    ...
    if sorted:
        ...
        sort_latency = (
            sort_passes * 2 * (pk + HW_PARAMS.fixed_dve_inst_overhead)
            + HW_PARAMS.sort_fixed_overhead
        )
    ...
    rotation_latency = (
        (ns - 1)
        * 2
        * div_ceil(lk, HW_PARAMS.rotation_tile_size)
        * HW_PARAMS.rotation_latency_per_tile
    )
...
def _calculate_rotational_constants(
    ...
    if orig_k <= _SMALL_K_THRESHOLD:
        return _calculate_rotational_constants_baseline(
            orig_k, vocab_size, pmax, BxS_tile
        )
...
def _find_optimal_tile_size(
    ...
    """
    if orig_k <= _SMALL_K_THRESHOLD:
        return _find_optimal_tile_size_baseline(
            orig_k, vocab_size, per_lnc_BxS, pmax, topk_sorted
        )
...
def _find_optimal_tile_size_baseline(
    ...
    for tile_size in range(1, pmax + 1):
        ...
        if stage_free_size > HW_PARAMS.max_free_dim:
            continue
        k_per_stage = get_ceil_aligned_size(
            div_ceil(orig_k, n_stages), HW_PARAMS.topk_per_stage
        )
        ...
        per_stage_cost = (
            div_ceil(k_per_stage, 8)
            * 2
            * (stage_free_size + HW_PARAMS.fixed_dve_inst_overhead)
        )
        unsorted_cost = n_stages * per_stage_cost
        needs_sort = topk_sorted or padded_k != orig_k
        if needs_sort:
            base_sort_cost = (
                div_ceil(padded_k, HW_PARAMS.dve_max_alus)
                * 2
                * (padded_k + HW_PARAMS.fixed_dve_inst_overhead)
            )
...
def _exceeds_concatenated_free_dim(
    orig_k: int, vocab_size: int, tile_size: int, pmax: int
) -> bool:
...
def create_rotational_topk_config(
    ...
    kernel_assert(
        len(inp_shape) == 2,
        f"rotated topk expects input to be 2D, actual was {len(inp_shape)}",
    )
    ...
    kernel_assert(
        BxS == topk_config.BxS,
        f"TopkConfig BxS dim {topk_config} does not match input BxS dim {BxS}",
    )
    ...
    if per_lnc_BxS > pmax or _exceeds_concatenated_free_dim(
        orig_k, vocab_size, per_lnc_BxS, pmax
    ):
        tile_size = _find_optimal_tile_size(
            orig_k, vocab_size, per_lnc_BxS, pmax, topk_config.sorted
        )
...
def build_rotation_matrix(block_size: int, num_blocks: int, dt: str):
    """Generate permutation matrix for rotational algorithm on-device.

    Builds a block-diagonal circulant shift-1 permutation matrix. Used to
    rotate local top-k results across batch partitions between stages, ensuring
    each stage sees candidates from different partitions.

    Args:
        block_size: Size of each block.
        num_blocks: Number of blocks.
        dt: Data type for matrix (e.g. nl.float16).

    Returns:
        [N, N] SBUF tensor where N = block_size * num_blocks.

    Example (block_size=3, num_blocks=2)::

        matrix = [[0, 0, 1, 0, 0, 0],    # row 0: 1 at col (0-1)%3 = 2
                  [1, 0, 0, 0, 0, 0],    # row 1: 1 at col (1-1)%3 = 0
                  [0, 1, 0, 0, 0, 0],    # row 2: 1 at col (2-1)%3 = 1
                  [0, 0, 0, 0, 0, 1],    # row 3: 1 at col 3+(0-1)%3 = 5
                  [0, 0, 0, 1, 0, 0],    # row 4: 1 at col 3+(1-1)%3 = 3
                  [0, 0, 0, 0, 1, 0]]    # row 5: 1 at col 3+(2-1)%3 = 4
    """
    N = block_size * num_blocks
    ident = nl.shared_identity_matrix(N, dtype=dt)
    matrix = nl.ndarray((N, N), dtype=dt, buffer=nl.sbuf)

    partition_stride = ident.get_pattern()[0][0]

    nisa.tensor_copy(
        dst=matrix.reshape((N, num_blocks, block_size))[:, :, 0 : block_size - 1],
        src=ident.ap(
            pattern=[
                [partition_stride, N],
    ...
    nisa.tensor_copy(
        dst=matrix.reshape((N, num_blocks, block_size))[
            :, :, block_size - 1 : block_size
        ],
        src=ident.ap(pattern=[[partition_stride, N], [block_size, num_blocks], [1, 1]]),
        engine=nisa.vector_engine,
    )
...
def build_stage_offsets(n_stages: int, bxs_size: int, stage_free_size: int):
    ...
    nisa.iota(
        lhs[:, 0:total_partition_dim],
        pattern=[[0, bxs_size], [stage_free_size, n_stages]],
    )
    offsets = nl.ndarray((padded, 1), dtype=nl.float32, buffer=nl.sbuf)
    for i in nl.affine_range(padded // 32):
        nisa.nc_transpose(
            dst=offsets[nl.ds(i * 32, 32), 0:1],
            data=lhs[0:1, nl.ds(i * 32, 32)],
            engine=nisa.vector_engine,
        )
...
def rotate(
    dst: nl.NkiTensor, tensor: nl.NkiTensor, rotation_matrix: nl.NkiTensor
) -> nl.NkiTensor:
    """
...
def insert(tensor: nl.NkiTensor, values: nl.NkiTensor, offset: int = 0) -> None:
    ...
    nisa.tensor_copy(
        dst=tensor[:, nl.ds(offset, num_values_to_insert)],
        src=values,
        engine=nisa.scalar_engine,
    )


def validate_topk_input(
    inp: nl.NkiTensor, n_fold: int = 1, local_top_k_per_stage: int = 0
) -> None:
    """
    ...
    kernel_assert(
        len(inp.shape) == 2,
        f"input has {len(inp.shape)} dims, topk expects input to be 2D",
    )
    kernel_assert(
...
def naive_scanning_topk(
    inp: nl.NkiTensor, topk_config: TopkConfig
    ...
    topk_indices = nl.ndarray(
        (BxS, k), dtype=HW_PARAMS.index_dtype, buffer=nl.shared_hbm
    )
    ...
    for tile_idx in nl.sequential_range(n_tiles):
        ...
        tile_batch_end = min(
            tile_batch_start + tile_size, min(lnc_batch_start + per_lnc_BxS, BxS)
        )
        ...
        nisa.dma_copy(
            dst=topk_values[hbm_slice, :], src=sbuf_topk_values[sbuf_slice, :]
        )
        nisa.dma_copy(
            dst=topk_indices[hbm_slice, :], src=sbuf_topk_indices[sbuf_slice, :]
        )
...
def topk_core(data: nl.NkiTensor, k: int) -> Tuple[nl.NkiTensor, nl.NkiTensor]:
    ...
    for fold_idx in nl.static_range(n_fold):
        if (k % HW_PARAMS.topk_per_stage != 0) and (fold_idx == n_fold - 1):
            val_buf = nl.ndarray(
                (BxS, HW_PARAMS.topk_per_stage), dtype=out_vals.dtype, buffer=nl.sbuf
            )
            ind_buf = nl.ndarray(
                (BxS, HW_PARAMS.topk_per_stage), dtype=out_inds.dtype, buffer=nl.sbuf
            )

            nisa.max8(dst=val_buf[...], src=data[:, :vocab_size])
            nisa.nc_find_index8(
                dst=ind_buf[...], data=data[:, :vocab_size], vals=val_buf
            )

            elts_remain = k % HW_PARAMS.topk_per_stage
            nisa.tensor_copy(
                dst=out_vals[:, k - elts_remain :],
                src=val_buf[:, :elts_remain],
                engine=nisa.scalar_engine,
            )
            nisa.tensor_copy(
                dst=out_inds[:, k - elts_remain :],
                src=ind_buf[:, :elts_remain],
                engine=nisa.scalar_engine,
            )

        else:
            i_free_dim = nl.ds(
                fold_idx * HW_PARAMS.topk_per_stage, HW_PARAMS.topk_per_stage
            )
