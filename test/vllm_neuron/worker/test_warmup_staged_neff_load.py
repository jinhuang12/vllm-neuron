...
"""The registered compiler runs in waves of ranks, signalling forward only.
...
JOIN_TIMEOUT = 30.0
RUN = "run_under_test"
ENDED_RUN = "run_that_already_ended"
...
def _flags(tmp_path, run=RUN, load="load_*", rank="rank_*"):
    """The flag files one run left behind, laid out as run/phase/bucket/load/rank."""
...
def _run(staged, group, world_size, park_after=None, still_inside=(), loads=1):
    ...
    A rank named in ``still_inside`` is not waited for: it is parked inside the compiler, and
    what matters is what the ranks behind it did while it was there. ``loads`` calls the staged
    compiler that many times per rank, as one warmup window does for the graphs it builds.
    """
...
def test_the_compilations_run_in_waves_of_the_configured_width(monkeypatch, tmp_path):
    """Two ranks compile at a time, in wave order, and every rank gets its result."""
...
def test_a_rank_that_never_returns_after_its_call_does_not_stall_the_waves_behind_it(
    ...
    """The wait points forward, so a rank stuck after its own call blocks nobody."""
    rows = _Rows()
    group = _FakeDist(WORLD)
...
def test_every_rank_writes_a_row_on_each_side_of_its_own_call(monkeypatch, tmp_path):
    """One entered row and one left row per rank, each naming its wave and RSS."""
    rows = _Rows()
    group = _FakeDist(WORLD)
    compiler = _FakeCompile(group, width=WAVE)
    staged = _install(monkeypatch, tmp_path, WAVE, group, compiler, rows)
...
def test_a_wave_size_at_the_world_size_stages_nothing(monkeypatch, tmp_path):
    """The gate turns itself off, with its reason, and writes no flag."""
    rows = _Rows()
    group = _FakeDist(WORLD)
...
def test_the_stage_calls_the_compiler_once_and_returns_its_object(monkeypatch, tmp_path):
    """The stage wraps the compiler, calls it once per rank, and hands back its result."""
    group = _FakeDist(WORLD)
    compiler = _FakeCompile(group, width=WAVE)
    staged = _install(monkeypatch, tmp_path, WAVE, group, compiler)
    threads, results, errors = _run(staged, group, WORLD)
...
def test_a_wave_that_never_signals_costs_the_next_wave_one_bounded_wait(monkeypatch, tmp_path):
    """The ranks behind wait once, log the missing rank, and carry on."""
    rows = _Rows()
...
def test_the_gate_is_off_with_its_reason_when_the_context_is_incomplete(monkeypatch, tmp_path):
    """No named load and no process group each turn the gate off by name."""
    rows = _Rows()
...
def test_a_second_load_in_the_same_window_waves_as_well(monkeypatch, tmp_path):
    """The second load waves on flags of its own, under one logged root."""
    rows = _Rows()
    group = _FakeDist(WORLD)
    compiler = _FakeCompile(group, width=WAVE)
    staged = _install(monkeypatch, tmp_path, WAVE, group, compiler, rows)
...
def test_flags_from_an_earlier_run_do_not_satisfy_this_runs_wait(monkeypatch, tmp_path):
    """A run signals under its own token and leaves an earlier run's flags alone."""
    rows = _Rows()
    group = _FakeDist(WORLD)
    compiler = _FakeCompile(group, width=WAVE)
    staged = _install(monkeypatch, tmp_path, WAVE, group, compiler, rows)
    ended = tmp_path / ENDED_RUN / "prefill" / "2048_kv2048" / "load_0"
    ended.mkdir(parents=True)
    for rank in range(WORLD):
        (ended / f"rank_{rank}.done").write_text("a run that already ended\n")
...
def test_the_backend_name_torch_compile_resolves_carries_the_stage(monkeypatch):
    """The registered backend is the stage, wrapping the vendor's compiler."""
    ...
    except BaseException:  # the vendor's compiler refuses a bare object; the row is what matters
        pass
...
def test_a_wave_size_of_zero_stages_nothing(monkeypatch, tmp_path):
    """Zero is the off switch: the gate says so and no rank waits."""
    rows = _Rows()
    group = _FakeDist(WORLD)
    compiler = _FakeCompile(group, width=WORLD)
...
def test_a_wait_timeout_of_zero_costs_one_row_and_no_waiting(monkeypatch, tmp_path):
    """A zero timeout logs a row per waiting rank and waits under a second."""
    rows = _Rows()
    group = _FakeDist(SMALL_WORLD)
    never_returns = threading.Event()
    compiler = _FakeCompile(group, park={1: never_returns})
