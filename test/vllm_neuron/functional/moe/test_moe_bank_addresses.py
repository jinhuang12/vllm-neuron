...
``nisa.tensor_scalar`` casts its int32 index data to float32 and back, so a bank row
address at or above 2**24 loses its low bits. The kernels address a weight bank in
contraction rows (``expert * H + h``), never in 128-wide column blocks, to stay under
that bound.
"""
...
def test_the_last_tile_of_the_last_expert_addresses_its_own_row():
    ...
    expected = _exact(experts - 1, stride, offset, step)
    assert got == expected, [
        (p, got[p], expected[p]) for p in range(TILE_SIZE) if got[p] != expected[p]
    ][:4]
    assert max(got) < EXACT_LIMIT and max(got) < experts * h_extent
    assert column == column_block * GATE_UP_SCALE_BLOCK


def test_an_address_above_two_to_the_24_rounds_in_the_engine():
    """The device's float32 rounding at an address above 2**24 -- why the address
    layout above stays in contraction rows."""
    _require_simulator()
    stride, offset, step = 4096 * 32, 3968 * 32 + 31, 32
    got = _read_back(287, stride, offset, step)
    expected = _exact(287, stride, offset, step)
    predicted = [_engine_int32(_engine_int32(287 * stride) + offset + step * p) for p in range(TILE_SIZE)]
    assert got != expected, (
        "the column-block view's address read back exact, so this geometry does not "
        "reach the float32 rounding it is chosen to show"
    )
    assert got[0] == 37744672 and expected[0] == 37744671
...
def test_eighteen_experts_route_the_last_block_to_the_last_expert():
    ...
    for expert in range(E18):
        ...
        expected = hidden[rows].to(torch.float32) @ weight
        scales64 = _expanded(grid, expert, H18, I18).to(torch.float64)
        expected64 = hidden[rows].to(torch.float64) @ (weights[expert].to(torch.float64) * scales64)
        assert torch.equal(expected64, expected.to(torch.float64)), (
            f"block {expert}: the fp64 and fp32 references differ"
        )
        assert float(expected.abs().max()) > 0.0, f"block {expert}: an all-zero reference measures nothing"
        equal_blocks += torch.equal(out[rows], expected)
