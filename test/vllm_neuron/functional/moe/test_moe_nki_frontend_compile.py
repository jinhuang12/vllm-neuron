...
The front end parses a kernel's own source before it places one tile, and it
refuses a call form outside its subset. The simulator never parses that source --
it runs the body as plain Python -- so a simulator run cannot see such a refusal.
This test compiles the three kernels instead, with fake operands of the served
shapes, in a child process that pins the platform target in its environment: the
toolchain then reads what it builds for from the environment, so the compile opens
no device node, and the child counts its own open device nodes to show it.
"""
...
# The child writes no bytecode: it may be the first thing to import the compiler
# in an environment, and a cache file left in a shared installation would be an
# unintended change to that installation.
_PIN = {"VLLM_NEURON_CPU_COMPILE": "1", "NEURON_PLATFORM_TARGET_OVERRIDE": "trn2",
        "PYTHONDONTWRITEBYTECODE": "1"}


def _emit(*fields: object) -> None:
    """Print one row on one line, keyed so the parent can pick its fields out."""
...
def _compile_each_kernel() -> None:
    ...
    for name, call in cases:
        ...
        try:
            with FakeTensorMode():
                call()
        except Exception as refusal:  # the front end's refusal is what the row carries
...
def _kernel_rows(printed: list[str]) -> list[dict[str, str]]:
    ...
    for line in printed:
        if not line.startswith(ROW + "|kernel="):
            continue
        # The diagnostic is the last field and it is the compiler's own text, which
        # may hold a pipe of its own, so it is cut off whole before the fields in
        # front of it are split.
