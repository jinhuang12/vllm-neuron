...
def _module():
    """The module under test, re-acquired the way another module's import does.

    Used to reach the cache and the split-out builder by name. Both are private:
    the counters are the public reading, and these two arms additionally check
    that the private state behind the reading behaves as the reading claims.
    """
    return importlib.import_module(_MODULE)


def test_scale_layout_is_built_once_per_weight_not_once_per_forward() -> None:
    """`B26-M2`, `inc-glm53f-076`: the operand leaves the per-forward path.

    WHAT WAS WRONG. The kernel scale operand is a function of the weight scale
    alone, but it was rebuilt on every entry into the seam -- element by element,
    on the device. In the served model that is three entries per shared-expert
    call, per layer, per decode step, all producing the same bytes.

    THE READING. ``builds`` is what the repair moves: one per distinct weight
    scale, on first use, and none on any later forward with the same weights.
    ``reuses`` is the same quantity seen from the other side, and the two must
    account for every bridge call, so a build that quietly stopped being counted
    cannot hide. These are counts reported as readings, not thresholds.
    """
    case = _build_case(uniform_one=True)
    scale = case["weight_scale"]
    ...
    reset_scale_layout_cache()
    reset_dispatch_counters()
    assert scale_layout_counters() == (0, 0)

    readings = []
    with _SimulatorCounter() as sim:
        for _forward in range(3):
            blockwise_fp8_mm(case["x"], case["weight"], scale)
            readings.append(scale_layout_counters())
    builds, reuses = scale_layout_counters()
    nki_dispatch, torch_fallback = dispatch_counters()
    print(
        f"[layout-reuse] per_forward={readings} builds={builds} reuses={reuses} "
        f"cache_entries={len(_module()._LAYOUT_CACHE)} "
        f"nki_dispatch={nki_dispatch} torch_fallback={torch_fallback} "
        f"sim_calls={sim.calls}"
    )

    # The route reading `-026` and `-033` declared does not move: three forwards
    # still enter the NKI seam three times. Only the layout builds collapse.
    assert (nki_dispatch, torch_fallback) == (3, 0), (
        f"the seam was entered {nki_dispatch} times with {torch_fallback} "
        f"fallbacks; this arm is about how often the OPERAND is built, so the "
        f"route reading must be unchanged for it to mean anything"
    )

    # The reading the lead's queue form asks for, stated per forward.
    assert readings == [(1, 0), (1, 1), (1, 2)], readings
    assert builds == 1, (
        f"the operand was built {builds} times for one weight scale across three "
        f"forwards; the repair is that it is built once"
    )

    # Conservation: every bridge call is accounted for as a build or a reuse, so
    # "builds stopped moving" cannot be a counter that stopped counting.
    assert builds + reuses == nki_dispatch == 3, (builds, reuses, nki_dispatch)

    # The cache holds one entry for one weight scale -- it does not grow per
    # forward, which would trade rebuilds for a leak.
    assert len(_module()._LAYOUT_CACHE) == 1

    # Nothing about the bytes moved: the memoised operand equals a fresh build.
    fresh = _module()._build_scale_layout(scale, scale_grid_shape(K, N))
    memoised = to_kernel_scale_layout(scale, K, N)
    assert torch.equal(memoised, fresh), (
        "the memoised operand differs from a freshly built one, so this repair "
        "changed the numbers it was only supposed to stop recomputing"
    )
    assert memoised is to_kernel_scale_layout(scale, K, N), (
        "two calls for the same weight scale returned different objects, so the "
        "operand is still being rebuilt"
    )


def test_scale_layout_cache_keys_on_identity_and_version_not_values() -> None:
    """What the cache keys on, read rather than declared.

    Keying on VALUES would put a full read of the scale grid back on the
    per-forward path and defeat the repair, so the key is the scale tensor's
    identity and version. Each arm below is the observable consequence of that
    choice, and each would fail if the key were the values.
    """
    grid = scale_grid_shape(K, N)
    first = torch.arange(K_BLOCKS * N_BLOCKS, dtype=torch.float32).reshape(grid) + 1.0

    reset_scale_layout_cache()
    to_kernel_scale_layout(first, K, N)
    assert scale_layout_counters() == (1, 0)

    # A DIFFERENT object holding IDENTICAL values must MISS. This is the arm
    # that separates identity keying from value keying: under a value key it
    # would hit, and `builds` would stay at 1.
    twin = first.clone()
    assert torch.equal(twin, first) and twin is not first
    to_kernel_scale_layout(twin, K, N)
    identical_values_reading = scale_layout_counters()
    assert identical_values_reading == (2, 0), (
        f"a distinct tensor with identical values read "
        f"{identical_values_reading}; a hit here would mean the cache is keyed "
        f"on values, which is the read this repair exists to avoid"
    )

    # An IN-PLACE write bumps torch's version counter, so the same object must
    # MISS rather than serve an operand built from the old numbers.
    before = scale_layout_counters()
    first[0, 0] = 99.0
    refreshed = to_kernel_scale_layout(first, K, N)
    after = scale_layout_counters()
    print(
        f"[layout-key] identical_values={identical_values_reading} "
        f"before_inplace={before} after_inplace={after} "
        f"cache_entries={len(_module()._LAYOUT_CACHE)}"
    )
    assert after == (3, 0), (
        f"an in-place write to the scale tensor read {after}; a hit here would "
        f"serve an operand built from the values before the write"
    )
    assert float(refreshed[0, flat_scale_index(0, 0, N_BLOCKS)]) == 99.0, (
        "the operand rebuilt after an in-place write does not carry the new "
        "value, so the version key is not doing what it claims"
    )

    # The same tensor at the same version still hits, so the misses above are
    # the key discriminating and not the cache simply never working.
    to_kernel_scale_layout(first, K, N)
    assert scale_layout_counters() == (3, 1), scale_layout_counters()

    # The reset is load-bearing: without it a later test would read a warm cache.
    reset_scale_layout_cache()
    assert scale_layout_counters() == (0, 0)
    assert len(_module()._LAYOUT_CACHE) == 0
