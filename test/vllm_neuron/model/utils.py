"""Host-side checkpoint-slice fakes for the model tests.

``docs/design/framework/model_bringup.md`` names three helpers and their
signatures, and this module provides them:
...
and drive the plugin's real weight loaders over them, with no checkpoint on disk
and nothing computed on an accelerator.

This module imports ``torch`` and nothing else. It never imports
``vllm_neuron``, because the loader is an input: the plugin attaches its loader
object to a parameter under the attribute name ``"weight_loader"`` and the load
call is ``loader.load(slices, rank)``. Both are consumed here by duck type off
whatever the caller passes in, so the helper stays device-free and importable
off-host while still driving the real loader objects when a caller supplies them.

``load_weights_from_slices`` reproduces the non-pipelined checkpoint path in
``vllm_neuron/utils/checkpoints.py`` step for step: ``mappings.get(name, name)``,
a scalar key normalised to a one-element list, missing keys reported per
parameter, checkpoint keys nothing references reported as unexpected, and
``tensor.to(device)`` on the loader's result. The slice interface mirrors what
the loaders actually call -- ``get_shape()`` plus ``__getitem__`` with a tuple of
slices.
"""
...
#: Attribute name the plugin attaches its loader under, from
#: ``vllm_neuron/utils/weight_loader.py``. Duplicated here as a string on
#: purpose: importing the plugin to read it would pull the device stack into a
#: host-side test.
...
class FakeSafeSlice:
    ...
    Presents the checkpoint-slice interface the plugin's weight loaders use --
...
def hf_state_to_fake_slices(
    ...
    key form the plugin's ``mappings`` dicts reference. Pass ``layer_idx=None``
...
class LoadFromSlicesResult(NamedTuple):
    ...
    Field meanings are that class's:
...
def _identity_load(slices: list[Any], rank: int) -> torch.Tensor:
    ...
    Mirrors ``SafetensorsWeightLoader.load`` with ``transform=None``, including
    its single-slice assertion and the ``.contiguous()`` on the way out.
    """
    if len(slices) != 1:
...
def _resolve_weight_loader(param: torch.nn.Parameter) -> Callable[[list[Any], int], torch.Tensor]:
    ...
    Accepts the plugin's loader object (anything exposing
    ...
    *,
    ...
    Args:
        module: the module whose ``named_parameters()`` are loaded. Loaders are
            read off the parameters themselves, so this is where the plugin's
            real loader objects enter -- attached by the model code under test.
