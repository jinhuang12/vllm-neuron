...
reading; the load releases those and nothing else. These tests load a miniature that
carries every class the load visits (routed bank, shared expert, dense MLP, MLA and
indexer) through the real loader on the CPU and check what the tree holds before and
...
#: The miniature's forward geometry. The loader admits whole consumer blocks, so
#: the residual width is one block wide and every MLP intermediate two, so each
#: grid has more than one block along both axes; the router selects exactly eight
#: experts per token, so the bank holds eight.
HIDDEN = 256
INTERMEDIATE = 512
ROUTED_EXPERTS = 8
TOP_K = 8
...
def _expected_released(module: torch.nn.Module) -> set[str] | None:
    ...
    reads what it loaded, ``None`` for a class outside the release's scope."""
    impl = _impl()
    if isinstance(module, impl.Glm5NextRoutedExperts):
...
def test_each_replaced_weight_is_resident_once_after_the_preps(
    ...
    """After the load every declared tensor is held once, as its prepared operand, and every class that declares nothing still holds what it loaded."""
    impl = _impl()
...
def test_the_forward_reads_the_operands_the_prep_stored(
    ...
    """The forward reads the objects the prep stored, byte-equal to the pre-release path's."""
    impl = _impl()
...
def test_nothing_in_the_tree_references_a_released_tensor(
    ...
    """Nothing in the tree still holds a freed tensor, and the dense MLP keeps its raw weights."""
    impl = _impl()
    directory = _written_checkpoint(tmp_path)
    recorder = _Recorder(impl._release_replaced_parameters)
    monkeypatch.setattr(impl, "_release_replaced_parameters", recorder)
...
def test_every_forward_is_byte_equal_with_the_release_on_and_off(
    ...
    """The MoE block, dense MLP, MLA projections and indexer stage on the released tree are byte-equal to a twin loaded with the release held off."""
    ...
    expected = _forwards(reference, inputs)
    identical = [name for name in got if _same_bytes(got[name], expected[name])]
...
def test_releasing_the_shared_expert_makes_its_forward_refuse(
    ...
    """Releasing the shared expert's six tensors makes the MoE-block forward raise from ``scale_route_operands``, which is why the class declares none."""
    monkeypatch.setenv("NKI_SIMULATOR", "1")
    impl = _impl()
    directory = _written_checkpoint(tmp_path)
    monkeypatch.setattr(
