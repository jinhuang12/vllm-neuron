...
whole module to meta. A bind for that route therefore receives meta operands, and
the tests here check what it does with them: the bind, the whole shape-only load
through the real loader, and preparation without value reads. The CPU arm is
tested beside the meta one, because the shipping load path must bind as before.
"""
...
def test_the_bind_completes_when_the_load_targets_meta() -> None:
    ...
    assert devices == {"meta"}, (
        f"the fixture did not put the six leaves on meta, so this would be the "
        f"CPU bind again: {sorted(devices)}"
    )
...
def test_a_cpu_load_binds_the_loaded_storage_as_before() -> None:
    ...
    assert (bound_sites, health["device"], shared) == (2, "cpu", 6), (
        ...
        f"{shared} of six parameters sharing the loaded storage; expected 2, "
        f"'cpu', 6"
    )
...
def test_a_loaded_but_unbound_layer_refuses_a_streams_call() -> None:
    ...
    with pytest.raises(impl.Glm5NextHyperConnectionError):
...
def _meta_load(tmp_path, name: str, *, with_bank: bool = False):
    """Run the real loader on meta through the shape-only reader; return the model and the reader.

    ``with_bank`` picks the routed fixture whose load completes: one dense MLP at
    layer 0 and three expert banks written at whole-block extents.
    """
    impl = _impl()
...
def test_the_hook_clears_both_refusals_the_forward_hits(
    ...
    """The embedding table matches the CPU load in shape and dtype, and every mHC layer has both sites.

    The root forward refuses when the table is ``None``. The fixture writes plain
    keys at a placeholder shape, so the reference is the same loader run on the
    CPU: an unset, reshaped or retyped table all fail against it.
    """
    impl = _impl()
    model = _lite_loaded(tmp_path)
    ...
    expected = None if reference_table is None else (
    ...
    assert table is not None, (
        "the embedding table is None after the hook, which is the condition the "
        "root forward refuses on"
    )
    assert got == expected, (
        f"the hook left the embedding table at {got} where the same loader on the CPU "
        f"leaves {expected} on the same checkpoint; a shape-only pass has to "
        f"reproduce the checkpoint's own header, in shape and in dtype"
    )
...
def test_the_meta_pass_skips_value_reads_and_packs_routed_banks(
    ...
    The fused route keeps each 128-wide quantization tile and needs no retile
    census. Its two packed tensors must still exist on meta after the six source
    tensors have been released.
    """
    ...
    assert compensated.count("compensate_block_scales") >= 1, (
        ...
        f"gate, so a pass that never reaches it shows nothing"
    )
