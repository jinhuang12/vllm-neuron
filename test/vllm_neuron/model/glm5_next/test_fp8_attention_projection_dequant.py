...
grid) must be the checkpoint's own product: the bytes as fp32 times the scale grid broadcast
over 128x128 blocks. Halving an e4m3 value is exact at or above 2^-5 and rounds to the nearest
2^-9 quantum below it, so under the squeeze the product is exact in the normal band and within
one quantum times the block scale elsewhere; with the squeeze off it is bitwise the checkpoint's.
Three readings through the weight map the model builds and the loaders it picks: a synthetic
weight and grid written under one sparse-attention leaf's real checkpoint keys, the same pair
with the squeeze off, and, when ``GLM53F_CHECKPOINT_DIR`` names the checkpoint, the real leaf.
"""
...
class Reading(NamedTuple):
    """One leaf through the model's loaders: the dequantised product, the checkpoint's own value, its raw pair, and the row printed."""
...
def _within_one_quantum(reading: Reading) -> tuple[bool, str]:
    """Whether the product is the reference up to the halving's rounding: ratio 1, one quantum times the scale per element, exact at or above 2^-5."""
...
def test_the_loaded_pair_dequantises_to_the_checkpoint_value_under_the_squeeze(synthetic, monkeypatch):
    """With the trn2 squeeze on, the product of the loaded weight and the loaded grid is the checkpoint's value up to one fp8 quantum below 2^-5 and exact above it."""
...
def test_the_real_leaf_dequantises_to_its_checkpoint_value_under_the_squeeze(monkeypatch):
    """The checkpoint's own leaf, when GLM53F_CHECKPOINT_DIR names the checkpoint, through the same route under the same bound."""
