"""Dispatch counters stay off the traced graph.

A compiled forward runs twice under ``fail_on_recompile``, so a counter stored from
inside a traced function would recompile the graph.
"""
...
def _router_call() -> tuple[tuple, dict]:
    """The fused router seam's arguments at the router test's declared extents."""
...
def test_the_router_seam_compiles_once_and_runs_twice():
    ...
    with _guard_log() as guard_lines:
        first, second, _breaks = _twice(router.noaux_tc_rmsnorm_router_topk, args, kwargs, fullgraph=True)
...
def test_an_in_trace_store_is_what_the_second_call_refuses(monkeypatch):
    """With the counter bump restored inside the traced function, the second call raises on recompile."""
    tiny_e2e._require_cpu_mode()
    ...
    assert listed, "the restored in-trace bump put no counter guard in Dynamo's list"
...
def test_the_miniature_forwards_compile_once_and_run_twice(tmp_path):
    ...
    for name, (fn, args, kwargs) in calls.items():
        with torch.no_grad(), _guard_log() as guard_lines:
            first, second, _breaks = _twice(fn, args, kwargs, fullgraph=False)
        flagged = _counter_guards(guard_lines)
...
def test_the_tiny_captures_compile_once_and_run_twice():
    ...
    for leg in rows.LEGS:
        ...
        with torch.no_grad(), _guard_log() as guard_lines:
            _twice(runner.model, (), translated, fullgraph=True)
        flagged = _counter_guards(guard_lines)
