...
from test.vllm_neuron.model.glm5_next.tiny import test_tiny_glm5next_request_tokens_133 as padded
...
def _hybrid_root():
    ...
    with torch.random.fork_rng():
        ...
        weights = kda._make_weights(
            int(cfg.hidden_size), 1, kda.DECLARED_KDA_HEAD_SIZE,
            kda.DECLARED_KDA_CONV_KERNEL_SIZE,
        )
...
def _snapshot(runner, end):
    """Read owned state and reject writes outside the request, except the pool trash row."""
...
def _compare(label, small, large):
    ...
    for name, got in small.items():
        want = large[name]
        gap = float((got.float() - want.float()).abs().max())
        print(f"PREFILL_STATE|{label}|{name}|exact={torch.equal(got, want)}|max_abs={gap:.8g}")
        if name.startswith("kda."):
            torch.testing.assert_close(
                got, want, rtol=kda.DECLARED_RTOL, atol=kda.DECLARED_ATOL,
                msg=f"{label}: {name}",
            )
        else:
            ratio, _, bound = padded._worst_offender(got, want)
...
def test_query_bucket_preserves_valid_logits_and_owned_state(
    ...
    for phase, index in (("prefill", 0), ("decode", 2)):
        gap = float((small[index] - large[index]).abs().max())
        print(f"PREFILL_STATE|real={real_tokens}|{phase}|rows={len(small[index])}|max_abs={gap:.8g}")
        # Same bound as the existing tiny generation's logit reference checks.
