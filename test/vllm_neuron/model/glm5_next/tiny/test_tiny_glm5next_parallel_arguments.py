...
caller and defaults them to the unsharded values, so a translation that supplied none would
make every rank of an expert-parallel serve run the first expert group. The runner's
translation is driven here under a stand-in parallel state, and the resolved values have to
reach the root at every call site and the expert bank behind it.
...
# The mesh this model is served on: 64 ranks in 16 expert groups of 4.
SERVE_WORLD_SIZE = 64
SERVE_EP_DEGREE = 16
...
# A mesh whose expert groups are one rank wide: a bank built for one rank runs unsharded
# under it, while the rank it receives is the last group's and not the default.
NARROW_WORLD_SIZE = 16
NARROW_EP_DEGREE = 16
NARROW_RANK = NARROW_WORLD_SIZE - 1

# The runner's model call sites: three warmups, execute, the idle dummy step and three
# graph captures.
RUNNER_MODEL_CALL_SITES = 8

# The one row the first resolve on a runner logs, once per rank of a serve.
LOG_ROW = re.compile(
...
def _install_state(monkeypatch, *, world_size: int, ep_degree: int, rank: int):
    ...
    it is constructed, and the bank here is built for one rank.
    """
...
def _translated_prompt(runner) -> dict:
    """The end-to-end prompt step, translated by the converter under test."""
...
def _resolved(call: dict, ep_rank: int, group: _StandInGroup) -> bool:
...
def test_below_degree_two_the_defaults_are_handed_over_explicitly(monkeypatch):
    ...
    assert int(parallel_state.get_neuron_ep_degree()) == 1, (
        "expert parallelism is never initialised here, so the live degree is 1"
    )
...
def test_the_translation_carries_the_three_arguments_and_the_root_binds_them(monkeypatch):
    ...
    assert sites.CARRIER_KWARG_KEYS == TRANSLATED_KEYS, (
        "the capture harness names a different six keys"
    )


def test_no_driven_site_reaches_the_root_with_the_defaults(monkeypatch):
    """Both graph captures and a real prompt step reach the root with the resolved values."""
    root, _caches, runner = _bound_runner()
    ep_rank, group = _install_state(
    ...
    with_defaults = [call for call in reached if not _resolved(call, ep_rank, group)]
...
def test_a_forward_through_the_translation_hands_the_bank_the_rank(monkeypatch):
    """The bank receives the resolved rank at every routed layer, and the logits do not move.
    ...
    with_defaults = [call for call in handed if not _resolved(call, ep_rank, group)]
