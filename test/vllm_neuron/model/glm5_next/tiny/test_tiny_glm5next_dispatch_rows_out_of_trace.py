"""Dispatch rows stay off the compiled graph, so the full-graph captures trace whole.

The runner captures each leg by calling ``torch.compile(model, backend=..., fullgraph=True)``
on the translated synthetic inputs. That entry is installed here with a backend that keeps
the graphs, and the runner's own prefill and decode captures are driven on the NKI arm. A
folded dispatch row (``@torch._dynamo.assume_constant_result``) is written on the host at
trace time; an unfolded one is a ``logging.Logger`` call inside the trace, which the
full-graph compile refuses. One test removes the fold and shows that refusal; a source
reader then requires every logger, print and warnings call on a shipped path to be folded.
...
def _full_graph(runner, leg: str) -> tuple[int, str | None]:
    """One leg through the compiled entry: the graphs kept and the refusal's first line."""
    ...
    return len(capture.graphs), refusal
...
def _classify(call: dict) -> str:
    """One of folded, load_time, fallback or shipped_unfolded; the last must stay empty."""
    if call["folded"]:
        return "folded"
...
def test_the_captures_trace_with_no_logging_break():
    """No graph break of either captured leg names a logging call."""
    runner = _runner()
    named: dict[str, list[str]] = {}
    for leg in LEGS:
        graphs, _breaks, reasons = _explain(runner.model, _translated(runner, leg))
...
def test_the_captures_compile_as_one_full_graph_each():
    ...
    for leg in LEGS:
        readings[leg] = _full_graph(runner, leg)
...
def test_an_unfolded_dispatch_row_is_what_the_capture_refuses(monkeypatch):
    ...
    logging_breaks = _logging_breaks(reasons)
    _count, refusal = _full_graph(runner, "prefill")
    assert logging_breaks, (
        "the unfolded row broke no graph, so a fold that went missing would not be seen"
    )
    assert refusal is not None and "logging" in refusal.lower(), refusal


def test_no_shipped_path_emission_stands_unfolded():
    """Every logger, print or warnings call on a shipped path is folded."""
    files, _excluded = _seam_files()
    calls: list[dict] = []
    for path in files:
