"""The runner's first request on the tiny root: warmup, one prefill, one decode, token ids.

A real ``NeuronModelRunner`` built from an engine config with no sampling or scheduling knob
set, its two warmups, then ``execute_model`` on a scheduler output, the way the worker calls
it. What is read here:

* the platform resolves on-device sampling and async scheduling off from the model class,
  naming the class in each row it logs, and refuses an explicit request for either;
* warmup leaves no async-execution state behind;
* the first request returns integer token ids from the vLLM Sampler over the full
  vocabulary, each the strict argmax of the model's logits and of the reference's, under a
  one-id head bias added outside the model;
* a request whose generation crosses into a block that is not its neighbour returns those
  same ids through three steps, the last of which attends the second block out of the bank
  rather than off the overlay. Two further readings discriminate the block table from a
  slice: the bank slot the table names holds that step's own latent while the page a slice
  reader would have used holds nothing, and handing the last step a slice reader's row moves
  the attention seam's output far outside this file's equality tolerance while the two steps
  before it stay bit-identical;
* the spec-to-non-spec transition is taken from the recorded fact, never without a
  speculative config and always when the fact and the config agree;
* a sampled-token future that holds logits is refused by name rather than reaching ``numpy``.

The last group runs against a class patched to declare an on-device sampler it does not
have, which is the only way to reach the async path at all.

    VLLM_NEURON_CPU_MODE=1 NKI_SIMULATOR=1 NKI_PRECISE_FP=1 python -m pytest \\
        test/vllm_neuron/model/glm5_next/tiny/test_tiny_glm5next_first_request.py
"""
...
REQUEST = "req-0"
# The prefill bucket and the decode batch the warmups compile.
PREFILL_BUCKET = tiny.STACK_TOKENS
DECODE_BATCH = 1
# vLLM keeps block 0 as the null block, so a scheduler hands out blocks from 1.
FIRST_BLOCK = 1
# The page the serving configuration asks for. At this page the whole prompt sits in one
# block and the generation crosses into the second, which is the crossing a served request
# makes. At this root's own page of four the prompt already spans thirty-two blocks and
# there is no crossing left to read.
SERVED_PAGE = 128
# Block numbers with holes between them, which a recycled pool hands out routinely. The gap
# is what makes the table a table: a reader that treated it as a slice would take the holes
# as content.
CROSSING_TABLE = (FIRST_BLOCK, FIRST_BLOCK + 3)
# The step that reads the second block out of the bank, and the reason a second decode is
# driven at all. The layer hands the kernel this step's own rows beside the window and the
# kernel overlays them after it copies the pages, so every row a step writes comes from the
# overlay and not from the page it lands in. At position 128 that row is the second block's
# only populated row, so the page the table's second entry names is never read there and the
# same logits come back whichever block the entry holds. One step later the row written is
# 129 and row 128 comes off the bank, out of that page. The selection reaches it whatever
# the scores say, because the indexer appends the incomplete final pool's own token indices
# and derives the tail itself: a sequence of 130 over a pool of four appends 128 and 129.
BANK_READ_POSITION = 129
# The block a reader that took the table for a slice would put after the prompt's, and which
# this request never names.
SLICE_BLOCK = FIRST_BLOCK + 1
# The sparse seam as it was imported, held so two watched runs cannot wrap each other.
...
# The head bias added to the root's logits outside the model: +8 at one of the two ids the
# seeded head scores equally (0 and 160). It is exact in bfloat16, so the argmax is that id
# with a margin far above the comparison tolerance.
...
def _parallel_state(tmp_path, vllm_config):
    """One rank over gloo with a file rendezvous, under the config vLLM's op layer reads."""
...
def _engine_config(*, async_scheduling: bool | None = None, on_device_sampling: bool | None = None,
    ...
    neuron_config: dict = {
        "num_batched_tokens_buckets": [PREFILL_BUCKET, e2e.E2E_MAX_SEQ_LEN],
...
def _declaring_a_sampler(monkeypatch) -> None:
    """Make the class declare an on-device sampler it does not have, so both knobs stay on."""
...
def _kv_cache_config(runner: NeuronModelRunner, num_blocks: int | None = None) -> KVCacheConfig:
    ...
    num_blocks = e2e.E2E_BLOCKS + 1 if num_blocks is None else num_blocks
...
def _head_bias(root) -> torch.Tensor:
    ...
    bias = torch.zeros(tiny.STACK_VOCAB_SIZE, dtype=torch.float32)
...
def _prefill_step(prompt: list[int], groups: int, table: tuple[int, ...] | None = None) -> SchedulerOutput:
    ...
    blocks = list(table) if table is not None else list(range(FIRST_BLOCK, FIRST_BLOCK + e2e.PROMPT_BLOCKS))
    new = NewRequestData(
        req_id=REQUEST,
        prompt_token_ids=prompt,
        mm_features=[],
        sampling_params=SamplingParams(temperature=0.0, max_tokens=e2e.GENERATED_TOKENS),
        pooling_params=None,
...
def _decode_step(position: int, generated: int, groups: int, page: int = tiny.MLA_PAGE_SIZE,
...
def _watching_the_seam(monkeypatch, plant: tuple[int, ...] | None = None) -> dict:
    """Record what each sparse call overlays and what it returns; optionally plant a row.

    The seam's own return is recorded because that is the tensor the block table feeds. A
    wrong page reaches the head through three layers and a residual stream, which dilutes
    it, while the layer's own output carries it undiluted. Both are keyed by the bank's
    ``data_ptr``, so a call pairs with the bank it overlays rather than with the order the
    layers happen to run in.

    ``plant`` replaces the request's own entries in the table row and leaves the bucket's
    padding as it arrived. The switch starts off, so the steps before the one under test
    write where the real table says and only the named step reads the planted row.
    """
...
def _crossing_run(tmp_path, root, prompt: list[int], *, monkeypatch=None, watch: bool = False,
    ...
    """One crossing request driven three steps: the prompt, the step that opens the second
    block, and the step that reads that block out of the bank.

    Returns the ids, the logits, the bank and what was tapped. ``plant`` turns the slice
    table on for the third step only, which is the one the bank serves.
    """
...
def _record_async_breaks(runner: NeuronModelRunner, monkeypatch) -> list[str]:
...
def test_an_explicit_async_request_is_turned_off_by_name():
    """vLLM resolves the async default before the platform hook, so an explicit request is
...
def test_the_runner_refuses_async_without_a_sampler_by_name(tmp_path, monkeypatch):
    """With a declared sampler, sampling off and async on, the runner refuses by class name."""
    e2e._require_cpu_mode()
...
def test_warmup_leaves_no_async_execution_state(tmp_path):
    ...
    e2e._require_cpu_mode()
    is_async, buffer, pending = _warmed_state(tmp_path, _engine_config(), e2e._fixture()["root"])
    assert is_async is False and buffer == {} and pending is False


def test_warmup_leaves_no_async_execution_state_with_async_on(tmp_path, monkeypatch):
    """With a declared sampler and async on, both warmups still leave the buffer empty."""
    e2e._require_cpu_mode()
    _declaring_a_sampler(monkeypatch)
    is_async, buffer, pending = _warmed_state(
        tmp_path, _engine_config(async_scheduling=True), e2e._fixture()["root"]
    )
    assert is_async is True and buffer == {} and pending is False


def test_the_first_request_returns_integer_token_ids(tmp_path):
    """No knob set: prefill then decode return one int each, the argmax of the biased
    logits and of the biased reference."""
    e2e._require_cpu_mode()
    ...
    for label, ids, logits, sequence in (
        ...
        assert tuple(logits.shape) == (1, runner.vocab_size) == (1, tiny.STACK_VOCAB_SIZE), (label, logits.shape)
        assert ids[0][0] == int(logits[0].float().argmax()), (label, ids)
        expected = e2e._reference_logits(
            fixture, torch.tensor(sequence, dtype=torch.int64)
        )[0].float() + bias
        top = logits[0].float().topk(2)
        margin = float(top.values[0] - top.values[1])
        tolerance = LOGITS_RTOL * float(expected.abs().max()) + LOGITS_ATOL
        ties = int((logits[0].float() == logits[0].float().max()).sum())
        assert ids[0][0] == int(expected.argmax()), (label, ids, int(expected.argmax()))
        assert ties == 1 and margin > tolerance, (label, top.indices.tolist(), margin, tolerance)
        torch.testing.assert_close(
            logits[0].float(), expected, rtol=LOGITS_RTOL, atol=LOGITS_ATOL
        )


def test_a_first_request_crossing_into_a_block_that_is_not_adjacent_returns_the_same_ids(tmp_path):
    """At the served page the prompt fills one block and the generation crosses into a
    second one that is not its neighbour.

    The table is what the layer reads a page number out of, so a table with a hole in it is
    the case a slice cannot serve, and the ids must still be the argmax of the reference
    under the same bias.

    Three steps and not two, which is what makes the second block's page load-bearing. The
    kernel overlays the rows a step writes onto the staged window after it copies the pages,
    so a step's own rows never come from a page: two steps would read the second block's
    only populated row off the overlay and pass on whichever block the table named. The
    third step writes 129 and attends 128 from the bank, out of the page the table's second
    entry names.

    The ids alone do not discriminate a table from a slice. Measured here, a slice reader's
    table moves this step's logits by 0.0078 against the 0.0130 this file compares at, so
    the head holds the wrong page below the tolerance even in a root of three layers. The
    two tests after this one read the bank slot and the seam's own output instead, where the
    difference is undiluted.
    """
    e2e._require_cpu_mode()
    fixture = e2e._fixture()
    root = fixture["root"]
    ...
    first_ids, second_ids, _third_ids = run.ids
...
def test_the_crossing_requests_row_lands_in_the_block_the_table_names(tmp_path, monkeypatch):
    """The bank row the third step attends is the second table entry's page, carrying
    position 128's latent.

    The step that opens the second block overlays its own row, so what shows the write
    reached the page the table named is the bank: the slot the runner's own formula gives
    for position 128 -- the table's second entry times the page -- holds that step's latent
    exactly, and the whole page a slice reader would have used instead holds nothing.

    The two generated latents are the same vector here, so no inequality between them is
    asserted: this root rotates nothing into the latent (``qk_rope_head_dim`` is 0) and both
    generated ids come back 160, so an inequality would grade the fixture rather than the
    table. What separates the table from a slice is the page.
    """
    e2e._require_cpu_mode()
    fixture = e2e._fixture()
    prompt = _prompt()
...
def test_the_crossing_logits_move_when_the_second_block_is_read_as_a_slice(tmp_path, monkeypatch):
    """Hand the third step a slice reader's table and the attention seam's output moves.

    A consumer that ignored the table and read the prompt's block plus the one after it
    would pass the two tests above unless reading that wrong page changes what comes back.
    So the same three steps run twice, both filling the bank through the real table, and in
    the second the last step is handed a row of consecutive blocks: the row the selection
    force-includes is then taken from a page this request never wrote.

    The seam's delta is graded against this file's own equality tolerance, which makes the
    reading a discrimination rather than a mere inequality: the wrong page moved the layer's
    output by 0.665 where that tolerance is 0.0246. The head's own delta is graded above
    zero only, because it measures 0.0078 against 0.0130 and stays inside the band -- which
    is why the seam and the bank are what discriminate here.

    The first two steps have to be bit-identical between the runs. The planted row is
    switched on for the last step alone, so a seam return or a logits row that differed
    earlier would mean the two runs diverged for a reason of their own and the last step's
    delta would grade that divergence instead of the table.
    """
    e2e._require_cpu_mode()
    fixture = e2e._fixture()
    root = fixture["root"]
    ...
    assert lying.planted, "no sparse call was handed the planted row, so nothing was read"
    ...
    for step in (0, 1):
        ...
        assert seam_same and head_same, (
            f"the runs already differed at step {step + 1}, before the planted row was "
            f"switched on, so the last step's delta would grade that divergence and not "
            f"the block table"
        )
...
def test_the_transition_is_read_from_the_recorded_fact(tmp_path, monkeypatch):
    ...
    with _parallel_state(tmp_path, config):
        ...
        reasons = _record_async_breaks(runner, monkeypatch)
...
def test_a_future_that_holds_logits_is_refused_by_name(tmp_path, monkeypatch):
    """With a declared sampler and async on, the first decode is refused, naming what the
    future holds."""
    e2e._require_cpu_mode()
    _declaring_a_sampler(monkeypatch)
    root = e2e._fixture()["root"]
...
def test_without_the_refusal_the_same_future_reaches_numpy(tmp_path, monkeypatch):
    """With the refusal removed, the same step raises a bfloat16 ``TypeError`` deeper in,
    from the rejection parser."""
    e2e._require_cpu_mode()
    _declaring_a_sampler(monkeypatch)
...
def test_the_transition_is_taken_when_the_fact_and_the_config_agree(tmp_path, monkeypatch):
    """A recorded spec step under a speculative config breaks the async flow at the
    transition and hands on the last accepted token."""
    e2e._require_cpu_mode()
    _declaring_a_sampler(monkeypatch)
    root = e2e._fixture()["root"]
    prompt = _prompt()
