...
Selecting this rank's experts from a python int would make the expert group a constant of
the captured graph, so an expert-parallel serve would compile one graph per group. The
runner hands the rank over as an int64 device tensor and the bank adds the rank's offset to
an ``arange`` from zero, which makes the group an input of the graph. A two-group tiny root
is traced at both ranks here and must give one graph, against the python-int form that
gives two; the tensor form must also compute what the int form computes, and no
``int(expert_parallel_rank)`` cast may remain in the model file.
...
def _pristine_fixtures(ranks) -> dict[int, dict]:
    ...
    return {rank: e2e._fixture() for rank in ranks}
...
def _two_group_root(monkeypatch, rank: int, fixture: dict):
    ...
    per_group = tiny.STACK_EXPERTS // GROUPS
    ...
    for index, layer in enumerate(root.model.layers):
        ...
        assert int(bank.num_local_experts) == tiny.STACK_EXPERTS, (index, bank.num_local_experts)
        ...
        for leaf in BANK_LEAVES:
            tiny._attach(bank, leaf, *sliced[leaf])
        built = bank.prepare_scale_operands(
            *tiny._prep_operands_from_the_module(bank, BANK_LEAVES, sliced)
        )
        assert built == 2, built
        rebound += 1
    assert rebound == tiny.STACK_LAYERS - tiny.STACK_FIRST_K_DENSE
...
def test_one_graph_serves_both_expert_groups(monkeypatch):
    ...
    assert not control_one_graph, (
        "the python-int form traced one graph at both ranks, so a graph traced per group "
        "cannot be told from a graph shared by them"
    )
...
def test_the_tensor_form_computes_what_the_int_form_computes(monkeypatch):
    ...
    for rank in GROUP_RANKS:
        ...
        assert mapped == tiny.STACK_LAYERS - tiny.STACK_FIRST_K_DENSE, mapped
...
def test_every_groups_first_expert_is_the_rank_times_the_group_size():
    """The offset form holds only while the partition is uniform and contiguous."""
    experts = int(Glm5NextTextConfig().n_routed_experts)
    checked = []
    for total, groups in ((experts, par.SERVE_EP_DEGREE), (tiny.STACK_EXPERTS, GROUPS)):
