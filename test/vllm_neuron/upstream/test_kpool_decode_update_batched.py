...
# PR #53906 adds to vLLM CORE. That package does not exist at this pin, so both limbs of the
# origin's ``current_platform.is_rocm()`` branch raise ``ModuleNotFoundError`` and the module
# could not be imported at all.
#
# THE THREE SYMBOLS ARE BOUND TO REFUSAL STUBS, NOT TO THIS PORT'S KERNELS, AND THAT IS A
# RULING RATHER THAN A SHORTCUT. Every test in this file ends in a bit-exact comparison of
# upstream's fp8 PAGED indexer cache. This port's k-pool path is bfloat16 by design and writes
# no such page, so no plugin callable carries these semantics: binding one would raise on the
# first call, and supplying the missing half here instead would put fork-authored torch on BOTH
# sides of the comparison, which proves nothing about the port. The whole module is therefore
# quarantined by design -- see ``pytestmark`` below -- and every stub raises if it is reached.
#
# RESOLUTION LEDGER: 3 of 3 upstream symbols resolved, disposition REFUSAL-STUB, 0 bound to a
# plugin callable. The count is stated rather than implied, so a symbol quietly dropped shows up
# as 2 of 3 rather than passing as a clean import.

#: The one message every stub raises with, so the three cannot drift apart.
_NOT_CARRIED = (
    "upstream fp8 paged k-pool cache write -- not carried by this port by design "
    "(design-20260905-u): this port's k-pool path is bfloat16 and writes no fp8 page"
)


def kpool_compress_and_write_cache(*args, **kwargs):
    """Upstream's prefill pool writer. Refuses; see ``_NOT_CARRIED``."""
    raise NotImplementedError(f"kpool_compress_and_write_cache: {_NOT_CARRIED}")


def kpool_decode_update_and_maybe_write_cache_batched(*args, **kwargs):
    """Upstream's batched decode updater. Refuses; see ``_NOT_CARRIED``."""
    raise NotImplementedError(
        f"kpool_decode_update_and_maybe_write_cache_batched: {_NOT_CARRIED}"
    )


def kpool_seed_tail_cache(*args, **kwargs):
    """Upstream's tail-ring seeder. Refuses; see ``_NOT_CARRIED``."""
    raise NotImplementedError(f"kpool_seed_tail_cache: {_NOT_CARRIED}")


#: Selection is gated for this whole module. ``test/vllm_neuron/upstream/conftest.py`` already
#: marks every item collected from this directory, and this line does not replace that: it
#: states the same thing inside the file because this file is ADAPTED rather than verbatim, so
#: its quarantine follows from its own design ruling and not only from where it sits.
pytestmark = pytest.mark.quarantined(
    reason="design-20260905-u: upstream's fp8 paged k-pool cache is not carried by this port"
)
