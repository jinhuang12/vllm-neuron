...
# Fork adapter, replacing the original's import branch and nothing else in this file.
#
# The original binds the three symbols below from ``vllm.models.glm5next``, a package that
# PR #53906 adds to vLLM itself. That package does not exist in the installed vLLM, so both
# limbs of the original's ``current_platform.is_rocm()`` branch raise ``ModuleNotFoundError``
# and the module cannot be imported at all.
#
# The three names are bound to stubs that raise rather than to this platform's own k-pool
# kernels. Every test in this file ends in a bit-exact comparison of upstream's fp8 *paged*
# indexer cache, and this platform's k-pool path is bfloat16 and writes no such page. Binding
# a local kernel would raise on the first call anyway, and supplying the missing half here
# would put fork-authored torch on both sides of the comparison, which proves nothing. Every
# item in this directory is skipped (see ``conftest.py``), so the stubs are never reached.

#: The one message every stub raises with, so the three cannot drift apart.
_NOT_CARRIED = (
    "upstream's fp8 paged k-pool cache write is not implemented here: this platform's "
    "k-pool path is bfloat16 and writes no fp8 page"
)
